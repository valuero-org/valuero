...
`test/functional/test_framework/test_framework.py:ValueroTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/valuero_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_valuerod = config["components"].getboolean("ENABLE_VALD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/valuero-org/valuero/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/valuero-org/valuero/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_valuerod):
        print("No functional tests to run. Wallet, utils, and valuerod must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if valuerod is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "valuerod"]) is not None:
            print("%sWARNING!%s There is already a valuerod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "VALD" not in os.environ:
        os.environ["VALD"] = build_dir + '/src/valuerod' + exeext
        os.environ["VALCLI"] = build_dir + '/src/valuero-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `valuero-cli help` (`rpc_interface.txt`).
