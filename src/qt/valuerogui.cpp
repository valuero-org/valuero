...
#include "valuerogui.h"

#include "valuerounits.h"
...
const std::string ValueroGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ValueroGUI::DEFAULT_WALLET = "~Default";

ValueroGUI::ValueroGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ValueroGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Valuero address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and valuero: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to VAL addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Valuero addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Valuero addresses"));
    ...
    openAction->setStatusTip(tr("Open a valuero: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Valuero command-line options").arg(tr(PACKAGE_NAME)));
...
void ValueroGUI::createToolBars()
{
    if(walletFrame)
    {
        /** VAL START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/valuerotext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* valueroLabelLayout = new QVBoxLayout(toolbarWidget);
        valueroLabelLayout->addWidget(label);
        valueroLabelLayout->addWidget(toolbar);
        valueroLabelLayout->setDirection(QBoxLayout::TopToBottom);
        valueroLabelLayout->addStretch(1);
...
void ValueroGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ValueroGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ValueroGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ValueroGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ValueroGUI::setWalletActionsEnabled(bool enabled)
{
...
void ValueroGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ValueroGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ValueroGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ValueroGUI::optionsClicked()
{
...
void ValueroGUI::showDebugWindow()
{
...
void ValueroGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ValueroGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ValueroGUI::openClicked()
{
...
void ValueroGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ValueroGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ValueroGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ValueroGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ValueroGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ValueroGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** VAL START */
void ValueroGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void ValueroGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void ValueroGUI::gotoManageAssetsPage()
{
...
void ValueroGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Valuero network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ValueroGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ValueroGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ValueroGUI::updateHeadersSyncProgressLabel()
{
...
void ValueroGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ValueroGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Valuero"); // default title
...
void ValueroGUI::changeEvent(QEvent *e)
{
...
void ValueroGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ValueroGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ValueroGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "VAL")
        msg += tr("Amount: %1\n").arg(ValueroUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(ValueroUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void ValueroGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ValueroGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ValueroGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ValueroGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ValueroGUI::setHDStatus(int hdEnabled)
{
...
void ValueroGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ValueroGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ValueroGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ValueroGUI::detectShutdown()
{
...
void ValueroGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ValueroGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ValueroGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ValueroGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ValueroGUI::subscribeToCoreSignals()
{
...
void ValueroGUI::unsubscribeFromCoreSignals()
{
...
void ValueroGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<ValueroUnits::Unit> units = ValueroUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ValueroUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(ValueroUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (ValueroUnits::Unit u : ValueroUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ValueroUnits::name(u)), this);
...
{
    setText(ValueroUnits::name(newUnits));
}
...
void ValueroGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=VALBTC"));
