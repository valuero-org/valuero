 ...
 * between valuerod, and valuero-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    FlushWallets();
#endif
    GenerateValueros(false, 0, Params());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), VAL_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/valuero-org/valuero>";
    const std::string URL_WEBSITE = "<https://valuero.org>";
...
/** Sanity checks
 *  Ensure that Valuero is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Valuero process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", VAL_CONF_FILENAME)).string());
    ...
    GenerateValueros(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
