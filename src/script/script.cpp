...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** VAL START */
    case OP_VAL_ASSET              : return "OP_VAL_ASSET";
    /** VAL END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_VAL_ASSET) { // OP_VAL_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == VAL_R) { // Check to see if VAL starts at 27 ( this->size() < 105)
                if ((*this)[28] == VAL_V)
                    if ((*this)[29] == VAL_N)
                        index = 30;
            } else {
                if ((*this)[28] == VAL_R) // Check to see if VAL starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == VAL_V)
                        if ((*this)[30] == VAL_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == VAL_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == VAL_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == VAL_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == VAL_R) {
