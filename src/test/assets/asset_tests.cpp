...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of VALCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("VAL", type));
        BOOST_CHECK(!IsAssetNameValid("VAL", type));
        BOOST_CHECK(!IsAssetNameValid("VALCOIN", type));

        //- Versions of VALCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("VAL.COIN", type));
        BOOST_CHECK(IsAssetNameValid("VAL_COIN", type));
        BOOST_CHECK(IsAssetNameValid("VALSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERVAL", type));
        BOOST_CHECK(IsAssetNameValid("VALSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEVAL", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_VALS", type));
        BOOST_CHECK(IsAssetNameValid("SEVALOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("VAL", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
