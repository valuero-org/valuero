...
The RPM spec file provided here is for Valuero-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://valuero.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Valuero git repository](https://github.com/valuero-org/valuero) but are not part of
...
Currently only `Source10` is used, for the example `valuero.conf` file.
...
in the valuero git repository. At present only an SVG version of the Valuero
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the valuero
...
needed when 0.12.1 is released, a proper fix is already in the Valuero git
master branch.
...
idea to build Valuero Core against a static version of that library compiled
...
The `valuero.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `valuero-server` package. At this point, most RPM based
...
The `valuero-server` package should have SELinux support. How to properly do
...
Valuero-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/valuero/). He has been
packaging Valuero for Fedora at least since 2012.
...
4. I renamed the `valuero` package that contains the Qt GUI to `valuero-core` as
that appears to be how the general population refers to it, in contrast to
`valuero-xt` or `valuero-classic`. I wanted to make sure the general population
